    ...
    public function all(array $query)
    {
        ...
        if (Auth::user()->role_id == 3) {
            $invoice = DB::table('invoices')->where('invoices.company_id', Auth::user()->company_id)
                ->select('invoices.id', 'invoices.keterangan', 'invoice_users.invoice_pdf', 'invoices.invoice_number', 'invoices.payment_charge', 'invoices.name', 'invoices.no_table', 'invoices.order_at', 'invoices.payment_status', 'invoices.payment_at', 'invoices.order_status')
